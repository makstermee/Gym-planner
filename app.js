...
let state = JSON.parse(localStorage.getItem('gym_planner_spa') || `{"username":"","theme":"light","plans":{},"logs":[]}`);
if(!state.plans){state.plans={};DAYS.forEach(d=>state.plans[d]=[]);}

const daySelect=document.createElement('select'); // możemy dodać wybór dnia w Planie
...
document.getElementById('addExercise').onclick = ()=>{
  ...
  if(!state.plans["Dzisiaj"]) state.plans["Dzisiaj"]=[];
...
function startRest(){if(interval) clearInterval(interval); timer=+document.getElementById('restSeconds').value||60; interval=setInterval(()=>{
  timer--;
  document.getElementById('timerDisplay').textContent=`${Math.floor(timer/60).toString().padStart(2,'0')}:${(timer%60).toString().padStart(2,'0')}`;
...
// Historia i eksport/import
const logArea = document.getElementById('logArea');
function renderLogs(){
  logArea.innerHTML='';
  state.logs.slice().reverse().forEach(l=>{
    const div=document.createElement('div'); div.className='card'; div.textContent=`${l.day} ${l.sets}x${l.reps} @ ${l.weight}kg`;
...
// Reset danych
document.getElementById('resetData').onclick=()=>{if(confirm("Na pewno chcesz zresetować dane?")){state={username:'',theme:'light',plans:{},logs:[]}; saveState(); renderPlan(); renderLogs(); applyTheme();}}

// Save state
function saveState(){localStorage.setItem('gym_planner_spa',JSON.stringify(state));}
